...
-- ──────────────────────────────────────────────────────────────────────
-- Win log (#3) — the brag file. See supabase/folio_wins.sql. DATA LINE: a win
-- is Chris's own work in his own words; no revenue/volume/shop figures.
create table if not exists folio_wins (
  ...
  source_ref  text,                    -- "project:<id>" / "task:<id>" — dedupe key
...
-- Pip pulls relevant past context by MEANING, not just recency.
-- Full design: docs/pip-architecture-f6-plan.md. Canonical: supabase/folio_embeddings.sql.
-- ──────────────────────────────────────────────────────────────────────
...
create table if not exists folio_embeddings (
  ...
  content_fingerprint text not null,        -- fnv1a(content) of the source row — skip re-embed when unchanged
...
-- Recall RPC — SECURITY INVOKER (RLS applies) + explicit user predicate (belt &
-- suspenders). Always called with the user-JWT client so auth.uid() resolves.
...
-- "Pip connected some dots" card reads the open ones. (Source: supabase/observations.sql)
...
create policy "obs_select_own" on folio_observations for select using ((select auth.uid()) = user_id);
drop policy if exists "obs_insert_own" on folio_observations;
create policy "obs_insert_own" on folio_observations for insert with check ((select auth.uid()) = user_id);
drop policy if exists "obs_update_own" on folio_observations;
create policy "obs_update_own" on folio_observations for update using ((select auth.uid()) = user_id);
