      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 4 }}>
          ...
          <button onClick={handleSave} disabled={!name.trim() || saving} style={{
            background: !name.trim() || saving ? GB : C.accent,
            border: "1px solid " + (!name.trim() || saving ? C.border : C.accent),
      ...
      <div ref={menuRef} style={{ position: "relative" }}>
        <button
          ...
          style={{
            ...
            background: open ? C.accent : GB,
            border: "1px solid " + (open ? C.accent : GB_BDR),
        ...
        {open && (
          ...
          >
            {/* User header */}
            <div style={{ padding: "12px 14px 11px", borderBottom: "1px solid " + C.border }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: C.text }}>
                {userMeta ? userMeta.full_name || "User" : "User"}
              </div>
              {userMeta && userMeta.title && (
                <div style={{ fontSize: 11, color: C.textMuted, marginTop: 1 }}>{userMeta.title}</div>
              )}
            </div>
