...
- The **daily-summary ingest extraction** (`/api/parse-digest`,
  `PIP_DIGEST_MODEL` override) runs on **Sonnet** — it's extraction
  with judgment (real commitment vs. journal noise), the same task
  class as meeting summarize; the data line is enforced in-prompt.
