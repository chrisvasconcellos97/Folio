...
- [x] (S,P2, NEEDS-GO) touches LIVE prod DB + verify not app-called first. DB: REVOKE EXECUTE FROM anon,authenticated on 6 SECURITY DEFINER RLS-helpers (folio_member_role_unchanged, folio_org_peer_user_ids, folio_user_org_ids, folio_user_writable_org_ids, gauge_owner_unchanged, rls_auto_enable).  — SQL staged — confirmed real via advisors; awaiting prod-apply OK
