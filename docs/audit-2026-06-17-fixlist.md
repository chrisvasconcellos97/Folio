...
- [x] (M,P2) item 48.3 — activity-gate pip-state-refresh triggers (cuts 70-90%); drop chat-open 20-stale sweep now operator-run covers state. **DONE (F2+F3, 2026-06-19, branch `claude/f3-pip-event-recompute-sxllsa`, NOT deployed — awaiting Chris's test + the migration). Two-tier gate:** client recency sweep (App.jsx Part 9 rewritten — no timer; signal = max(last_interaction_at, meeting.updated_at, task.updated_at) > context_checked_at; widened from meeting-only to task add/close/edit) + server fingerprint skip (`computeContextFingerprint` in accountContext.js, time-stable, +7 tests; pip-state-refresh skips the Haiku call when unchanged, `force` bypasses). PipView's chat-open sweep removed (App's sweep supersedes it). F2: `context_struct`/`context_fingerprint`/`context_checked_at` persisted (migration `supabase/pip_context_fingerprint.sql` — APPLY TO PROD). Plan: `docs/pip-architecture-f3-plan.md`.
...
- [x] (L) **THE ONE THING** — single `buildAccountContext()` layer feeding chat/brief/summarize/operator (kills the parity-drift bug class permanently). ~week. See X6. **DONE (F1, 2026-06-18, branch `claude/app-audit-strategy-hhcrz2-4ix1ho`, NOT deployed — awaiting Chris's test):** new pure `src/lib/accountContext.js` (no supabase) renders the per-account context for all three surfaces via per-surface presets (chat/brief/summarize/operator) that make the intentional omissions + depth explicit in ONE map; chat (`pipContext.renderContextProse`), summarize (`pip.js` bp3), and operator-run (`api/operator-run.js`) all route through it; legacy `renderAccountFull` + 7 summarize render-helpers + the local operator renderer deleted. Drift lock: `accountContext.test.js` (25 parity assertions incl. cross-surface checks). F4 discipline preserved (static system blocks byte-stable; builder output is the per-account cached tail). F2/F3/F5/F6 still open below.
...
- [~] (M) Event-driven recompute generalized (item 48 levers): pip-state-refresh gating, conditional output fields, on-demand draft emails, skip roll-up on quiet nights. **pip-state-refresh gating DONE (F2+F3, 2026-06-19 — see above).** STILL OPEN (operator-run levers, item 48 (4)(5)(6)): conditional output fields (emit agenda/delta only when relevant), on-demand draft emails, skip the roll-up on a genuinely quiet night.
- [x] (L) Pip agent loop (tool_result round-trip, chat only). X6 F5. **DONE (F5, 2026-06-19, SHIPPED to `main`):** server-side loop in the chat handler — model emits a read tool → server executes (RLS-scoped) → tool_result → model continues → final answer. New read-only tools `lookup_account` (renders via the F1 shared `accountContext`), `find_open_work`, `search_notes`; existing action tools stay client-side + terminal (confirm-card gate untouched). Pure control logic + tool defs in `src/lib/pipAgentTools.js` (15 tests); I/O in `api/_pipAgentLoop.js`. Bounded: ≤4 model calls (`PIP_AGENT_MAX_STEPS`), forced `tool_choice:none` on the last step, `logPipUsage` per call; kill switch `PIP_AGENT_LOOP=off` → byte-identical single-shot. Read tools never cross to the client. Plan: `docs/pip-architecture-f5-plan.md`.
- [x] (L) pgvector semantic recall. X6 F6. **DONE (2026-06-19, SHIPPED to `main` — needs `OPENAI_API_KEY` in Vercel to activate; inert without it):** `folio_embeddings` (pgvector, RLS, hnsw) + `match_folio_embeddings` RPC; `api/embed-sync.js` fingerprint-gated daily backfill of the user's notes/summaries/project-notes/updates (OpenAI text-embedding-3-small via `api/_embed.js`, key-optional); recall wired into chat via `api/pip.js#attachRecall` and rendered through the shared `accountContext.js` `recall` section (per-account) + `pipContext.js` global lane. Plan: `docs/pip-architecture-f6-plan.md`. +8 tests. Recall over BOTH user notes AND Pip summaries (broader than the audit's "summaries only" note).
