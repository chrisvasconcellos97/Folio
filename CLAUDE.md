...
- ✅ **Email Roundup Import (June 2026)** — paste-and-parse pipeline from work email into Folios with no email integration, OAuth, or IT approval needed. User's work Claude artifact summarizes the inbox daily into a structured roundup (looks like personal notes — no Folios vocabulary); user copies it and pastes into the "Import email roundup" entry in HomeView's quick-capture menu. `api/email-import.js` (Haiku, auth-guarded) parses the text against the user's account roster and contact list, returning structured JSON. `src/lib/threadKey.js` normalizes email subjects (strips RE:/FW: prefixes, lowercases) and computes fuzzy match ratio for deduplication. `src/lib/emailImportApply.js` applies FK-safe: creates contacts → upserts threads → creates tasks → inserts thread events → updates thread status. Four action types: **Action** → `folio_tasks`, **Committed** → `folio_tasks` with `is_commitment=true`, **Waiting** → thread `status='waiting'`, **Logged** → activity entry. Subject line stored as thread identifier — future pastes match by `(subject_norm, account_id)` and append rather than duplicate. New contacts flagged `NEW` in the artifact are auto-created with name + account + email pre-filled. HomeView gains a "Waiting on N replies" panel (threads where `status='waiting'`, sorted oldest-first). Pip context parity: open email threads block wired into both `pipContext.js` (Ask Pip chat) and `pip.js` (meeting summarize). `useEmailThreads` + `useThreadEvents` hooks with realtime sync. New tables: `folio_email_threads` (thread identity spine with status machine) + `folio_thread_events` (timeline per thread); `source_thread_id` + `source` columns added to `folio_tasks`. SQL: `supabase/folio_email_threads.sql` — **run in production Supabase**.
