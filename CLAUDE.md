...
11. **Export & sharing:**
    ...
    **Bug fix — silent new_task failures in pipPlanApply** — When Pip returns a `new_task` row that references a `project_id` that doesn't exist (or returns `new_task` with no project_id at all), `applyPipPlan.js` line 91 silently drops the task with "Project not found" — no error surfaces to the user. Fix: when `ensureStage(row.project_id)` returns null, fall back to creating a `new_item` (same as the `new_item` case) instead of failing silently. Use `row.title` as the text. The user's tasks land somewhere visible (Items tab) rather than being lost. Second hardening: the summarize prompt in `pip.js` should only return `new_task` rows when `project_id` is a known active project ID from the injected projects list — if no project match, return `new_item` instead.

    **Escalate task → Gauge project + pull meeting tasks in** — Any task in the account Items tab (or in PipSummarizePreview before Apply) can be promoted to a Gauge project. Two actions on each item row:
    1. **"→ New project"** — opens `ProjectModal` pre-filled with the task title as project name + account already set. Creates a net-new project.
    2. **"→ Add to project"** — dropdown picker of all active projects for the account. Sort order: projects whose `created_at` is within the current session (last ~2 hours, same account) float to the top as "From this meeting" — so a project just created during this meeting is the obvious first pick. Remaining open projects follow sorted by recency. Selecting any project sets `folio_tasks.project_id` on the task, which moves it out of the Items tab (`.is("project_id", null)` filter) and into the Gauge project board.
    Any task from the same meeting (matching `source_meeting_id`) gets the same two actions, so you can batch-move related tasks into one project quickly. No new tables — `folio_tasks.project_id` already exists.

    **Auto-generated meeting titles from summary** — After a meeting is summarized, if the title is still the system-generated default (`"{Cadence label} — {date}"` or blank), Pip proposes a short descriptive title based on what was discussed. No extra API call — add a `suggested_title` field to the existing `summarizeDraftPip` response alongside `plan[]`. `PipSummarizePreview` shows an editable title field at the top of the modal pre-filled with the suggestion (e.g. *"Parts Authority — invoice feed delay + integration timeline"*). User can edit inline or leave as-is; the value writes to `folio_meetings.notes` (the title column) on Apply. If the meeting already has a custom non-default title, skip the field entirely — don't overwrite intentional titles.
...
- ✅ **Gauge project auto-status from task completion** — `src/lib/gaugeUtils.js` exports `computeAutoStatus(stages, isStanding, taskStatusColumns, currentStatus)`. For discrete projects, counts `completed_at` on stages + sub_stages; for standing/kanban projects, counts tasks in the last `task_status_columns` lane. Returns `planned` (0 done) → `in_progress` (some done) → `complete` (all done). Never touches `blocked` or `on_hold` — those remain manual overrides. `ProjectModal` auto-syncs status via `useEffect([stages, isStanding])` and shows a read-only badge with an "(auto)" label instead of a dropdown for the three auto-values; two small toggle buttons (Blocked / On Hold) appear for manual overrides with a "↩ Back to auto" escape. `StandingBoardView.commitTask()` and `deleteTask()` include the computed status in every `onUpdate` call so standing-board task moves also trigger auto-status. `handleSave()` re-derives status from stages as a belt-and-suspenders before writing to DB.
- ✅ **Gauge task reordering** — ↑ / ↓ arrow buttons on each task row in `ProjectModal` (discrete projects). `moveStage(idx, dir)` swaps adjacent stages in the array. First row's ↑ and last row's ↓ are disabled/dimmed. Allows placing a newly added task in the right step order without deleting and re-adding.
- ✅ **Add contacts during account creation + unified role flags** — `AddAccountModal` gains an inline contacts section (new accounts only): stage as many contacts as needed (name, title, email, phone + POC/Leader/Primary toggles) before saving; after the account is created and returns its ID, all staged contacts are batch-inserted into `folio_contacts`. `AddContactModal` now includes the `is_leader` and `is_primary` toggles that were previously edit-only — creation and edit forms are now in full parity on all three role flags (POC / Leader / Primary). No schema changes.
