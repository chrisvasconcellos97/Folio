...
## Session Handoff — June 6 2026 (read first)
...
34. **Pip Autonomous Operator — nightly loop + materialized operator state + dynamic Pip cards everywhere** — *(Phases 1 & 2 BUILT June 8 2026 — committed locally, NOT yet deployed; awaiting "ship it")* turn Pip from a pull-triggered advisor into a scheduled agent that works the book of business overnight and pre-fills every Pip surface in the app. The morning brief is the read-only *head* of this; the loop is the body. **Design locked:**

    - **Phase 1 status (built):** the engine + the Home operator report. Built: `api/operator-run.js` (cron endpoint — CRON_SECRET-gated, service-role, runs every morning, signal-gated deep passes capped at 8/run on Sonnet **run in parallel waves of 4** (`maxDuration:60`; sequential calls blew past Vercel's function timeout and killed the run after ~5 accounts before it wrote the report — fixed June 8 2026), one portfolio roll-up call, propose-only); **(weekend opt-in gate was built then PULLED June 8 2026 at Chris's request — caused testing confusion and saved only pennies; the loop now runs daily, signal-gating alone controls cost);** `vercel.json` cron at `0 9 * * *` (~5am ET); schema (`supabase/pip_operator.sql` — operator_* columns on `folio_pip_account_state` + new `folio_operator_reports` table, **applied to prod via MCP + folded into schema.sql**); `src/hooks/useOperatorReport.js`; HomeView "Pip · Operator Report" card (prioritized plan + drafted-follow-up Copy/Mail) that **suppresses the live daily brief when a report with content exists** (no double-spend); registered in `scripts/test-api-imports.js`; docs + PDFs. **On deploy (when Chris ships):** set Vercel env `CRON_SECRET` (endpoint 401s without it); optional `OPERATOR_TZ` (default America/New_York) + `PIP_OPERATOR_MODEL` (default claude-sonnet-4-6). `SUPABASE_SERVICE_ROLE_KEY` + `ANTHROPIC_API_KEY` already set. The cron only begins firing once `main` is deployed. Manual test after deploy: `GET /api/operator-run?secret=<CRON_SECRET>` (optionally `&user=<uuid>`) returns a per-user run summary.

    - **Phase 2.5 (built — unified Pip card + Home redo, June 8 2026):** shared `src/components/PipCard.jsx` shell — one anatomy everywhere: HEAD (✦ Pip label + timestamp, a high-level **headline**, a density row of count chips) always visible; BODY collapsed by default ("Show details ▾"). `OperatorPanel` composes it (head = `operator_headline` + `N risks · draft · N moves`; body = situation/delta/risks/draft/moves). **Amber unread-notification glow** (`pip-unread` + `pipUnreadPulse` keyframe, `--c-yellow-glow` token both themes): card pulses amber + "NEW" dot when `operator_generated_at` is newer than this device's last-open (localStorage `folio_op_read_<acct>`), clears on expand. New per-account `operator_headline` column (prompt + migration applied to prod + schema.sql + pip_operator.sql). **Home redo:** operator REPORT output restructured to `{ headline, opening, sections:[{kind:fire|watch|win|signal, items:[{account_name, line, action, has_draft}]}] }` (stored headline→headline, opening→report_prose, sections→plan_items); HomeView renders a **fused sectioned briefing** (Pip-voiced opening paragraph + sectioned tappable rows with Open + inline "✦ Draft ready" buttons — drafts/commitments fused into rows, no separate cards); the centered hero orb now uses a **heartbeat pulse** (`.pip.heartbeat` thump-thump-rest, reduced-motion-gated) and the **hero headline is the operator's real Pip-voiced headline** (never the canned "quiet day" when there's work); commitment-nudge + live-daily-brief cards suppressed when an operator report has content. **Pip personality** dialed up in `REPORT_SYSTEM` (dry wit + slightly anxious + loyal, with examples + a never-say-filler rule) concentrated in the headline + opening; section rows stay tight. Fallback (no operator state yet) = lightweight heuristic head-only card on Overview. **Not yet propagated:** the unified PipCard + glow onto Cadence/Gauge/Home-as-PipCard (Home uses its own fused briefing for now); Calendar strip; PWA push.
