...
- **Before shipping items 7–10:** do a full layout audit first — review placement, spacing, and information hierarchy across every screen to make sure new features land cleanly into a well-organized foundation
...
2. **UX polish — two remaining fixes:**
   - **Actionable empty states** — add a CTA button to each empty state: AccountsView "No accounts" → "Add Account", ItemsTab "All clear" → "Add Action Item", MeetingsView → "Log a Meeting", ContactsTab "No contacts yet" → "Add Contact".
...
3. **Accessibility (a11y) — three remaining fixes:**
   - **aria-labels on remaining icon-only buttons** — calendar nav `‹` `›` and ChipDropdown trigger buttons still missing aria-label.
   ...
   - **aria-live on error containers** — form errors don't have `aria-live` announcement. Add `aria-live="polite"` to error containers.

4. **Code quality — two remaining fixes:**
   - **Consolidate rgba opacity scale** — `rgba(74,155,130,0.*)` is used at 6, 7, 12, 15, 18, 3, 35, 4, 45 inconsistently. Define a standard scale in `src/lib/colors.js`.
   - **Split oversized files** — CadenceView.jsx (571 lines), AccountsView.jsx (553 lines), and OverviewTab.jsx (531 lines) each mix 3–5 sub-components inline. Extract CalendarView, WeekView, and ListView out of CadenceView as a starting point.

5. **Error resilience — one remaining fix:**
   - **Fix fire-and-forget metadata updates** — `last_meeting` and `last_interaction_at` updates in useMeetings, useItems, and useContacts use `.then()` with no `.catch()`. Failures are completely silent. Add error logging at minimum.

6. **Feature completeness:**
   - **⚠️ DATA LOSS — Add `attendees` column to meetings table** — LogMeetingModal saves `attendees` array, MeetingsTab and CadenceTab render it, but `folio_meetings` has no `attendees` column. Every meeting's attendee list is silently dropped on save. Fix: `ALTER TABLE folio_meetings ADD COLUMN IF NOT EXISTS attendees text[];`
   - **pip_email mailto link** — pip_email is displayed with a Copy button in MeetingsTab but no `mailto:` link. ContactsTab already uses `href={"mailto:" + c.email}` — apply the same pattern.
   ...
   - **Schema sync** — `supabase/contacts_v2.sql` adds phone/email/linkedin columns missing from the base `schema.sql`. Anyone running schema.sql fresh is missing those columns. Consolidate into schema.sql.

7. **Native feel / micro-polish:**
...
8. **Overview tab redesign + account intelligence:**
...
18. **Gauge — full build (multi-user, custom columns, request workflow):**
    ...
    - **Completion feeds change log** — completing a task automatically creates a dated entry on the linked Folio account's change log (item 17 above).

19. **Route Builder — territory routing for MSO field visits:**
