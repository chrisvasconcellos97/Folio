...
10. **Onboarding & contextual help:**
    ...
    **Phase 2 — The "few questions a week" drip:**
    ...
    - **Terminology gap lane (Lane C)** — Pip notices a proper noun/brand appearing ≥3 times across meeting notes that isn't a known account name, contact name, or glossary entry, and asks about it. Example: *"You keep mentioning Keystone — is that a new account or tied to another?"* User answers *"That's LKQ's aftermarket brand"* → answer saves directly to `folio_pip_facts` as a glossary entry (rides the existing facts injection path, immediately visible to Pip in all future outputs). Unlike structural gaps, this lane requires a lightweight Haiku batch scan of recent meeting notes to extract unknown proper nouns — runs weekly, not daily, to stay cheap (~$0.01/month). Candidates above a frequency threshold generate `folio_pip_questions` rows with `category='terminology'`, `source='gap_observed'`, `trigger_context` holding the detected term + meeting count. This is how Pip builds up your company's vocabulary — brands, internal codenames, distributor names, program names — without you having to think to add them to the glossary manually. **Shipped + improved June 2026:** `/api/detect-terminology` (Haiku) now runs **daily** (was weekly) and **account-anchors** each question — finds the account whose notes mention the term most and asks *"You keep mentioning Fuse5 around John's Auto Parts — what is it? A system they use, a brand, a program, or a person?"* instead of a context-free prompt. Answers ride `onTermLearned` → `folio_pip_facts`.
    ...
    - Throttle (revised June 2026 per Chris feedback — "up his questions to a handful a day so I can catch up"): soft cap of **5 per rolling 24h**, no weekly cap, no skip cooldown — answer one and the next surfaces immediately so you can power through several in a sitting. `DAILY_CAP` in `usePipDripQuestions.js`. Persisted in `folio_pip_questions` status + timestamps.
    - **Evergreen generic question bank REMOVED (June 2026).** The filler "interview" questions ("What does a great week look like?", "Who do you report to?") read as a personality quiz, not Pip piecing things together from real data — Chris called them garbage. `seedEvergreenIfEmpty` is no longer called; `purgeEvergreenQuestions` runs in the daily gap pass to delete any still-queued. Observation-driven gaps + terminology are now the only source. If detectors find nothing, the drip is simply silent (better than filler).
    ...
    **Phase 3 — Polish:**
    - Haiku-generated questions for novel gaps the templates miss. *(Now Sonnet 4.6 as of June 2026 — see Pip model tiering in Already shipped.)*
    - **Question generator context + prompt enrichment (queued June 2026 — the "free other-half lever").** Upgrading `/api/generate-questions` to Sonnet raised the *floor* on reasoning, but the generator is still fed a compact structured summary (account lines + theme counts + known facts) and a fairly rigid prompt. The compounding win is giving the now-Sonnet generator more to reason over and more room to do it: (a) widen the input — recent meeting-note excerpts (not just theme counts) for the most-active accounts, relationship power-map (champions/blockers), tone trend, open commitments, recent corrections/lessons, so questions can connect threads the way Pip does in chat; (b) loosen the prompt — fewer template-shaped constraints, explicit license to ask the genuinely insightful cross-account question, with the cheap clarifier questions as a secondary lane rather than the default shape; (c) keep the cost guardrails intact (backlog soft-cap, weekly cadence, token ceiling on the widened input). Goal: the question that made Chris notice the gap should be the *typical* output, not the lucky one. No schema changes — pure prompt/context work in `generate-questions.js` (+ the caller's payload assembly). Parity note: any new context field pulled in here should already exist in `pipContext.js`; reuse those renderers rather than re-deriving.
...
32. **Schedule future meetings on the calendar (one-off scheduled meetings)** — requested June 2026. Today the Calendar only renders recurring **cadence** occurrences (`getOccurrencesInRange`); there's no way to put a single future meeting on a specific date. "Log Conversation" creates a notes-first meeting that lands in History, not a scheduled item. The gap: a lightweight "schedule a meeting on a date/time" that shows on the calendar and converts to the normal meeting-notes flow when it's time. **Proposed design (not locked):**
    ...
    - **Scope LOCKED (June 2026):** account-required (searchable picker) + date + time + method + optional agenda note; both entry points (click-a-day prefills date AND a "+ Schedule meeting" button); reuse `useCadenceReminders` 30m/5m/start nudges + show in Home "today". Build approved.

33. **Project builder — searchable, multi-account account picker** — requested June 2026. When building a Gauge project (`ProjectModal`), the account field still requires scrolling rather than searching. Chris wants to **search accounts, click to add one, then search again** to add more (a project can touch multiple accounts). Item 26 ("Universal account picker search") rolled `AccountPicker` into ProjectModal for the single-account field, so check whether ProjectModal is on the old `<select>` or whether this is specifically the multi-account case. Design: a multi-select built on the existing `AccountPicker` (search → pick → chip → search again), chips removable; persist the set (new join or an account_ids array on `gauge_projects` — check current single `account_id` model first). Confirm whether projects should become truly multi-account in the data model or just multi-tag for filtering.
