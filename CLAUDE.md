...
21. **Display name in dropdowns + contacts as assignees** — Two related fixes to owner/assignee pickers everywhere in the app (task owners, project owners, Pip plan assignee dropdowns, account owner pickers):
    - **Display name instead of email** — Chris currently shows up as his raw email address when he selects himself. Pull `full_name` (or `display_name`) from `folio_org_members` / Supabase Auth user metadata and show that instead. Fall back to the part before `@` if no name is set. Affects every ChipDropdown + assignee `<select>` that renders org member email strings.
    - **Contacts as assignee options** — Since it's a solo org for now, the assignee pool is just Chris. Expand all owner/assignee pickers to also include `folio_contacts` for the relevant account (or all contacts for global pickers like quick tasks). Contacts appear below the org-members section, labeled "Contacts". Selecting a contact stores their name (not an auth UUID) in the `assignee_email` / `owner` field — the existing text field on `folio_tasks` already accepts free text so no schema change needed. Pip assignment hints already normalize `task_pattern` + `assignee_email` as text strings, so this rides the existing path cleanly.
