...
**Open follow-ups:** (1) `API_FOOTBALL_KEY` + scores validation. (2) Propagate the unified PipCard + glow onto Cadence/Gauge (they still use Phase-2 read-only blocks). (3) Calendar plan strip + PWA push (both deferred). (4) The git "unverified commits" stop-hook nags every turn — it's a local-verification artifact (commits ARE SSH-signed); not chased (re-pushing won't fix it and costs deploys).
