...
11. **Export & sharing:**
    ...
    **Escalate task → Gauge project + pull meeting tasks in** — Any task in the account Items tab (or in PipSummarizePreview before Apply) can be promoted to a Gauge project. "→ Promote to project" action on each item row: opens `ProjectModal` pre-filled with the task title as project name + account already set. Once the project exists, related tasks from the same meeting (those with the same `source_meeting_id`) can be added into the project's task list via an "Add to project" picker — dropdown of active projects on the account, click to move the task from the Items tab into a project stage. This lets Chris take a complex action item ("resolve integration issue") and escalate it to a tracked multi-step project, then pull the related sub-tasks from the same meeting directly into it. No new tables — `folio_tasks.project_id` already exists; setting it moves the task from the Items tab (`.is("project_id", null)`) into the Gauge project board.
