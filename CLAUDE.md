...
30. **In-meeting flow — capture without navigating (Chris's #1 pain point)** — The single biggest friction in the app: while live in a meeting, taking notes AND completing/editing project or task state at the same time forces context-switching that steals attention from the person across the table. Today, to act on a project mid-meeting you leave the note stream, find the project in the sidebar, expand it, edit a task — a multi-step detour while you're supposed to be listening. The thesis: **you should never have to leave the capture stream during a meeting.** Everything you'd want to do to a task/project should be doable from within the notepad via lightweight inline actions (slash-commands or quick-action chips: `/done catalog feed`, `/task send Danielle the deck`, `/due Friday`), combined with entity detection (item 27) auto-resolving who, and Pip's summarize plan handling the rest after. Sub-threads to scope: (a) in-stream slash-commands / quick actions on the meeting notepad; (b) one-tap task state changes (mark done, reassign, set due) without opening a project; (c) faster manual project/task editing outside meetings too — the editing flow itself is heavy; (d) **note-taking layout / capture ergonomics** — the notepad surface is awkward to navigate while typing; (e) **capturing the right structure** — turning what was said into the right tasks/owners/dates takes too much manual thought in the moment (item 27 entity detection helps here). **Discuss direction with Chris before building — this is a core-experience redesign, not a feature bolt-on.**

    **Candidate sub-features (ranked, from the 2026-06 friction convo — Chris named: acting mid-meeting, note-taking layout, capturing structure):**

    Root problem: the notepad is a dumb text box next to a separate "management" surface (sidebar). Every time Chris needs structure or action, he has to leave the typing flow and operate the other surface. Fix = make the notepad itself the place structure + action happen, so he never leaves the stream. Sidebar demotes from "where you act" to "ambient context you glance at."

    - **T1.1 — Slash-commands in the notepad** *(acting mid-meeting)* — `/task send Danielle the deck`, `/done catalog feed`, `/due Friday`, `/contact`, `/project` typed inline create/complete/date the task without touching the sidebar; entity detection (item 27) resolves *who*. Highest-leverage piece of Item 30.
    - **T1.2 — Inline checkboxes → real tasks** *(capturing structure)* — a line written as `[ ] follow up on pricing` renders as a live checkbox in the notes AND becomes a `folio_task` on summarize. The notes ARE the task list — kills post-meeting cleanup.
    - **T1.3 — Focus mode + sticky agenda** *(note-taking layout)* — toggle collapses the sidebar entirely (just the page); Pip brief / agenda pins to the top so talking points stay visible while scrolling. "Capture mode."
    - **T2.1 — Pre-filled meeting templates** — Pip seeds the page before the meeting starts: last meeting's open items, standing agenda, Decisions / Actions / Follow-ups headers. Structure before you type.
    - **T2.2 — Topic headers → per-topic summaries** — `## Pricing` chunks the meeting; Pip summarizes each section separately instead of one blurry block. Pairs with Tab-nesting (30a).
    - **T2.3 — Due-date + decision detection** — extend entity detection so "by Friday", "before the QBR", "we decided to…" get flagged in the moment, same as names.
    - **T3.1 — Inline quick-edit OUTSIDE meetings** *(the "editing the project myself" half)* — `TaskDetailPanel` is a multi-click detour to change one field. Click a field right on the card / kanban and edit in place, no panel. Separate surface from the meeting flow but same "don't make me navigate to act" thesis.
