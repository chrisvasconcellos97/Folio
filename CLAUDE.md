...
3. **Feature completeness:**
   ...
   - **Gauge — Standing Projects + Custom Columns + Admin Queue (locked spec):** Chris's Excel-replacement workflow. Today's Gauge projects are discrete (start → stages → complete). This adds a "standing" mode for never-ending request queues (e.g. LKQ shop NDA integrations as they roll in), with per-project custom columns so any team can model their own intake process.
     - **Naming:** rename existing concept `stages` → `tasks` (or `steps`) everywhere user-facing. DB column name `stages` stays for backwards compat — just the UI label changes. Each task is one line that can be assigned, tracked, and have data filled in by an admin.
     - **Schema:** add `is_standing boolean default false` to `gauge_projects`. Add `custom_field_schema jsonb` to `gauge_projects` (defines columns: array of `{ key, label, type, options? }`). Add `custom_fields jsonb` to each task in the existing `stages` array (values keyed to the schema). Add `account_id uuid` to each task (optional — links the task to a specific account, e.g. one shop in LKQ's integration queue).
     - **Standing vs discrete:** a discrete project has a start/end and tasks complete in sequence. A standing project never closes; tasks can be added at any time and represent independent incoming requests. UI surfaces the two differently — discrete shows a linear stage view, standing shows a board/table of tasks.
     - **"Bones" custom fields** (auto-included in every new project as defaults; user can remove or add more): Priority (dropdown: High/Med/Low), Owner (person, auto-filled to project creator), Submission Date (auto = task created_at), Due Date (date), Description (long text), Related Link (URL).
     - **Custom field types for v1:** Text, Long text, Number, Date, Dropdown (with options), Person (org member), Checkbox, URL.
     - **Admin queue:** assigning a task to an org member surfaces it in their existing "My Queue" filter in Gauge. Queue rows show: project name (always visible at the top of each row so admin knows which workflow), task title, custom field values inline (priority, due date, etc.). Updating fields in their queue writes back to the task; project view reflects progress.
     - **Admin → project context:** every queue row has a "View project" affordance (link/button or click the project name) that opens the parent project so the admin can see the full description, other tasks in the workflow, and the standing-project board state. Returning from the project should bring the admin back to their queue with state preserved. Optional grouping: queue can be toggled to group-by-project so an admin working through one project's intake sees its tasks clustered.
     - **Drill-down:** clicking a task in the project list opens a detail panel with every custom field editable, status, history (last updated), and a link to the linked account if any.
     - **Task editor UX (locked):** one unified detail panel handles task creation, editing, and admin work. When you click `+ New task` on a standing project board, the panel opens with title + bones fields + every custom field + assignee + account. Fill what you know now; leave admin fields blank. Save → task lands on the board AND in the assignee's queue. The same panel opens when admin clicks a task in their queue — they fill in their fields and save. Assignment is just one field — there's no separate "tag a person" popup; setting Assignee + saving is what triggers the queue notification.
