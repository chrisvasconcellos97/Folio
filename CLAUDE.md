...
**39. Pip speed — stream the meeting summary as it writes** — *(queued June 2026, Chris picked the approach)* The slowest user-facing Pip surface is meeting **summarize** (`mode:"summary"`, Sonnet, `max_tokens:3072`, **buffered** — you hit "End & Summarize" and stare at a spinner until the whole structured plan is generated). Chris's pick: make Pip's written recap **stream onto the screen word-by-word** (like Ask Pip chat already does via `streamPip`) so he's reading within ~1s instead of waiting. Keep Sonnet quality (don't downgrade to Haiku; don't hard-cap length).
