...
34. **Pip Autonomous Operator — nightly loop + materialized operator state + dynamic Pip cards everywhere** — *(Phase 1 BUILT June 8 2026 — committed locally, NOT yet deployed; awaiting "ship it")* turn Pip from a pull-triggered advisor into a scheduled agent that works the book of business overnight and pre-fills every Pip surface in the app. The morning brief is the read-only *head* of this; the loop is the body. **Design locked:**

    - **Phase 1 status (built):** the engine + the Home operator report ship together. Built: `api/operator-run.js` (cron endpoint — CRON_SECRET-gated, service-role, per-user weekend gate in local TZ, signal-gated deep passes capped at 12/night on Sonnet, one portfolio roll-up call, propose-only); `vercel.json` cron at `0 9 * * *` (~5am ET); schema (`supabase/pip_operator.sql` — operator_* columns on `folio_pip_account_state` + new `folio_operator_reports` table, **applied to prod via MCP + folded into schema.sql**); `src/hooks/useOperatorReport.js`; HomeView "Pip · Operator Report" card (prioritized plan + drafted-follow-up Copy/Mail) that **suppresses the live daily brief when a report with content exists** (no double-spend); registered in `scripts/test-api-imports.js`; docs (product-overview + upgrades + ai-governance) + PDFs. **On deploy (when Chris ships):** set Vercel env `CRON_SECRET` (endpoint 401s without it); optional `OPERATOR_TZ` (default America/New_York) + `PIP_OPERATOR_MODEL` (default claude-sonnet-4-6). `SUPABASE_SERVICE_ROLE_KEY` + `ANTHROPIC_API_KEY` already set. The cron only begins firing once `main` is deployed. Manual test after deploy: `GET /api/operator-run?secret=<CRON_SECRET>` (optionally `&user=<uuid>`) returns a per-user run summary. **Phase 2 (not built):** convert Cadence Hub / Gauge / Account-Brief-Me / Calendar cards to read the materialized state (the "dynamic cards everywhere" half); optional PWA push companion.
