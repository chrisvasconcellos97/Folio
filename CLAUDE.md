...
- ✅ **Assignee/recipient pickers — workspace-grouped + free-text escape hatch (June 2026)** — Chris could "only assign to myself" (solo user, no linked-account contacts) and the dropdowns weren't easy to scan. New shared `src/components/PersonPicker.jsx` used for every Gauge person field: groups options **account contacts first** (one optgroup per linked account, in order), **then My Team** (org members), **then everyone else** grouped by workspace (account) alphabetically — plus a "✎ Someone else…" option that swaps to a free-text name/email input (with a "☰ List" button back), so you're never locked to a fixed list even with no contacts/teammates. Wired into `TaskDetailPanel` (assignee + recipient + custom person-type columns) and `ProjectModal` (project-level "Assigned To" + per-task "Assign to…"). `ProjectModal` now receives `allContacts` from GaugeView so the "others" groups populate. Stored-value conventions preserved per surface (contact name vs email||name) to avoid changing existing data. Partial delivery/upgrade of item 21 (contacts as assignees).
