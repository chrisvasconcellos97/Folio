...
2. **Explicit registration in `src/main.jsx`** — uses `virtual:pwa-register` with `immediate: true`. The auto-reload signal is a **`navigator.serviceWorker.controllerchange`** listener — `onNeedRefresh` does NOT fire when `skipWaiting + clientsClaim` are set (the SW activates silently with no waiting state). On controllerchange (skipping the first one, which is the fresh-page first-control), we show a brief "Updating Folios…" toast and reload after 400ms. Folios autosaves notes / drafts / items, so silent reload is safe. Also forces `registration.update()` on startup, every 5 min, and on every tab-visibility change. Never remove the controllerchange listener; never re-add a manual refresh button without explicit reason.
